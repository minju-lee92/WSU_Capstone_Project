...
server =  function(input, output, session) {
    ...
    output$Delivery_Plot <- renderPlot({
        ...
        p <-ggplot(data=filtered, aes(x=as.factor(input$Time_unit))) + geom_line(aes(y=totalEarly), colour='blue',group =1 ) +
            geom_line(aes(y=totalLate), colour='grey',group=1) + ylab(label = "count") + xlab("Time")
