...
customer_info = read.csv("../../Data/customerInfo.csv")
customer_info <- customer_info[-c(1)]

table_a = read.csv("../../Data/tableA.csv")
table_a <- table_a[-c(1:2)]

table_b = read.csv("../../Data/tableB.csv")
table_b <- table_b[-c(1:2)]

table_d = read.csv("../../Data/tableD.csv")
table_d <- table_d[-c(1:2)]

table_e = read.csv("../../Data/tableE.csv")
...
filtered <-
          Customer_typeC %>%
          group_by(Overall.Customer.ID,Date) %>%
          filter(as.Date(Date) >= '2016-01-01',
          as.Date(Date) <= '2016-06-01',) %>%
          summarise(totalRequests = sum(Requested),g
...
```
```{r}
#min(as.Date(Customer_typeC$Date))
#max(as.Date(Customer_typeC$Date))
Day <- as.Date(Customer_typeC$Date, format = "%m/%d/%y") 

Customer_typeC <- Customer_typeC %>% mutate(Day = as.Date(Customer_typeC$Date, format = "%Y-%m-%U") , Month = Day <- as.Date(Customer_typeC$Date, format = "%Y-%m") , Year = as.Date(Customer_typeC$Date, format = "%Y"))

head(Customer_typeC)
```
```{r}
Customer_typeC <-Customer_typeC %>%
                            mutate(Week = format(as.Date(Date), "%Y-%m-%U"), Month = format(as.Date(Date), "%Y-%m"), Year = format(as.Date(Date), "%Y"))
        
head(Customer_typeC)    
```

```{r}
            filtered <- Customer_typeC %>%
                         group_by(Month) %>%
                         summarise(
...
ggplot(data=filtered, aes(x=Month)) + geom_line(aes(y=totalEarly), colour='blue',group =1 ) +
       geom_line(aes(y=totalLate), colour='grey',group=1)
names(filtered[,1])
as.name('Week')
```
```{r}

ggplot(data=filtered, aes(x=Month)) + geom_line(aes(y=totalEarly), colour='blue') +
       geom_line(aes(y=totalLate), colour='grey')
```
